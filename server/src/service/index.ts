...
import { findDocumentLinks } from './findDocumentLinks';
...
export function getVls(): VLS {
  ...
  return {
    ...
    configure(config) {
      const auicodeValidationOptions = config.auicode.validation;
      validation.uiml = auicodeValidationOptions.uiml;
      validation.css = auicodeValidationOptions.style;
      validation.postcss = auicodeValidationOptions.style;
      validation.scss = auicodeValidationOptions.style;
      validation.less = auicodeValidationOptions.style;
      validation.javascript = false;//auicodeValidationOptions.script; //js不校验
    ...
    findDocumentLinks(doc, documentContext) {
      ...
      languageModes.getAllModesInDocument(doc).forEach(m => {
        ...
        pushAll(links, findDocumentLinks(doc, documentContext));
      });
      return links;
    },
