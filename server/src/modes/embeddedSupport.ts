...
import { createScanner } from './uiml/parser/htmlScanner';
import { TextDocument, Position, Range } from 'vscode-languageserver-types';
import { TokenType, Scanner } from './uiml/parser/htmlScanner';
...
export interface AuiDocumentRegions {
...
export type EmbeddedType = 'ui' | 'script' | 'module' | 'style' | 'custom';
...
export function getDocumentRegions(document: TextDocument): AuiDocumentRegions {
...
function scanTemplateRegion(scanner: Scanner, text: string): EmbeddedRegion | null {
  ...
  while (unClosedTemplate !== 0) {
    ...
    if (languageId !== 'uiml' && start !== 0) {
      token = scanner.scanForRegexp(/<\/ui>/);
    ...
    if (start === 0) {
      ...
      if (token === TokenType.StartTag && scanner.getTokenText() === 'ui') {
        unClosedTemplate++;
      } else if (token === TokenType.EndTag && scanner.getTokenText() === 'ui') {
        unClosedTemplate--;
        // test leading </ui>
...
function getLanguageAtPosition(document: TextDocument, regions: EmbeddedRegion[], position: Position): string {
  ...
  return 'javascript';//'aui'; // 默认设置为javascript
}
