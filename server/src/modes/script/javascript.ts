...
import { AuiDocumentRegions, LanguageRange } from '../embeddedSupport';
...
export function getJavascriptMode(
  ...
  const jsDocuments = getLanguageModelCache(10, 60, document => {
    const auiDocument = documentRegions.get(document);
    return auiDocument.getEmbeddedDocumentByType('script');
  });

  const regionStart = getLanguageModelCache(10, 60, document => {
    const auiDocument = documentRegions.get(document);
    return auiDocument.getLanguageRangeByType('script');
  });
  ...
  return {
    ...
    doComplete(doc: TextDocument, position: Position): CompletionList {
      ...
      const completions = service.getCompletionsAtPosition(
        ...
        offset,
        {
          includeExternalModuleExports: _.get(config, ['auicode', 'completion', 'autoImport']),
      ...
      const entries = completions.entries.filter(entry => entry.name !== '__auiEditorBridge');
      return {
        isIncomplete: false,
    ...
    doResolve(doc: TextDocument, item: CompletionItem): CompletionItem {
      ...
      if (details) {
        ...
        if (details.codeActions && config.auicode.completion.autoImport) {
    ...
    format(doc: TextDocument, range: Range, formatParams: FormattingOptions): TextEdit[] {
      ...
      const defaultFormatter =
        scriptDoc.languageId === 'javascript'
          ? config.auicode.format.defaultFormatter.js
          : config.auicode.format.defaultFormatter.ts;

      if (defaultFormatter === 'none') {
        return [];
      }

      const needIndent = config.auicode.format.scriptInitialIndent;
...
function getSourceDoc(fileName: string, program: ts.Program): TextDocument {
  ...
  return TextDocument.create(fileName, 'aui', 0, sourceFile.getFullText());
}
