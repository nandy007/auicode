...
export function isAui(filename: string): boolean {
  return path.extname(filename) === '.aui' || path.extname(filename) === '.bui' || path.extname(filename) === '.bcp';
}

export function parseAui(text: string): string {
  const doc = TextDocument.create('test://test/test.aui', 'aui', 0, text);
...
export function createUpdater() {
  ...
  return {
    createLanguageServiceSourceFile(
      ...
      if (isAui(fileName) && !isTSLike(scriptKind)) {
        modifyAuiSource(sourceFile);
      }
      return sourceFile;
    },
    updateLanguageServiceSourceFile(
      ...
      if (isAui(sourceFile.fileName) && !isTSLike(scriptKind)) {
        modifyAuiSource(sourceFile);
      }
      return sourceFile;
    }
...
function modifyAuiSource(sourceFile: ts.SourceFile): void {
  ...
  if (exportDefaultObject) {
    ...
    const auiImport = setZeroPos(
      ts.createImportDeclaration(
        undefined,
        undefined,
        setZeroPos(ts.createImportClause(ts.createIdentifier('__auiEditorBridge'), undefined as any)),
        setZeroPos(ts.createLiteral('aui-editor-bridge'))
      )
    );
    const statements: Array<ts.Statement> = sourceFile.statements as any;
    statements.unshift(auiImport);

    // 2. find the export default and wrap it in `__auiEditorBridge(...)` if it exists and is an object literal
    ...
    const aui = ts.setTextRange(ts.createIdentifier('__auiEditorBridge'), {
      pos: objectLiteral.pos,
      end: objectLiteral.pos + 1
    });
    (exportDefaultObject as ts.ExportAssignment).expression = setObjPos(ts.createCall(aui, undefined, [objectLiteral]));
