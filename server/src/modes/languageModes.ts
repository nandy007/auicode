...
import { getDocumentRegions, AuiDocumentRegions } from './embeddedSupport';
import { getAuiMode } from './aui';
import { getCSSMode, getSCSSMode, getLESSMode, getPostCSSMode } from './style';
import { getJavascriptMode } from './script/javascript';
import { getUIMLMode } from './uiml';
...
export function getLanguageModes(workspacePath: string | null | undefined): LanguageModes {
  const documentRegions = getLanguageModelCache<AuiDocumentRegions>(10, 60, document => getDocumentRegions(document));
  ...
  let modes: { [k: string]: LanguageMode } = {
    aui: getAuiMode(),
    uiml: getUIMLMode(documentRegions),//getUIMLMode(documentRegions, workspacePath, jsMode),
