...
export function getStylusMode(documentRegions: LanguageModelCache<AuiDocumentRegions>): LanguageMode {
  ...
  return {
    getId: () => 'stylus',
    configure(c) {
      baseIndentShifted = _.get(c, 'auicode.format.styleInitialIndent', false);
      config = c;
    },
    onDocumentRemoved() {},
    dispose() {},
    ...
    format(document, range, formatParams) {
      if (config.auicode.format.defaultFormatter.stylus === 'none') {
        return [];
      }
      ...
      // Add one more indentation when `auicode.format.styleInitialIndent` is set to `true`
